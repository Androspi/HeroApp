...
export class CharactersService {
  ...
  constructor(
    private charactersService: RestCharactersService
  ) { }
  ...
  load(filters?: Partial<{ limit: number; offset: number; orderType: string; find: string; }>) {
    this.charactersService.get(filters).subscribe(({ results, total }) => {
  ...
  replace(filters: Partial<{ limit: number; orderType: string; find: string; }> = {}) {
    ...
    this.charactersService.get({ ...filters, limit }).subscribe(({ results, total }) => {
